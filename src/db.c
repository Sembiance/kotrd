...
sh_int  gsn_bfire;
sh_int  gsn_bacid;
sh_int  gsn_blightning;
sh_int  gsn_bfrost;
sh_int  gsn_bgas;

/* New Skills */
sh_int  gsn_heighten_senses;
sh_int  gsn_chameleon_power;
...
sh_int          gsn_reptilian;
sh_int          gsn_kender;
sh_int          gsn_drow;
sh_int          gsn_magespeak;
sh_int          gsn_high_elven;
sh_int          gsn_undead;
sh_int          gsn_thieves_cant;
sh_int          gsn_handtalk;
sh_int          gsn_forestsign;
sh_int          gsn_nephilim;
*/
...
void boot_db( void )
{
  ...
  /* OLC */     else if ( !str_cmp( word, "AREADATA" ) ) new_load_area(fpArea);
	...
	// Explorer/Killer Percentages
	init_area_mob_lists();
	init_area_room_lists();
	init_area_object_lists();
    }

    gTopKillers = 0;
    gTopExplorers = 0;
    gTopTreasureHunters = 0;
    do_updatetop(0,0);
...
void load_area( FILE *fp )
{
    ...
    pArea->area_flags   = fread_flag( fp ) | AREA_LOADING;
...
void save_maxes( void )
{
  FILE *fp;
  if ( !(fp=fopen(MAXES_FILE, "w")) )
    {
    bug( "Save_Maxes: could not open '%s'.", (int)MAXES_FILE );
    return;
    }
  fprintf( fp, "%d %s~\n", atm_players, date_atmp );
  fclose( fp );
  return;
}   
...
void load_old_mob( FILE *fp )
{
	...
	kill_table[URANGE(0, pMobIndex->level, MAX_LEVEL-391)].number++;
    }
...
void load_old_obj( FILE *fp )
{
	...
	pObjIndex->material_type	= fread_number( fp );
...
void load_rooms( FILE *fp )
{
	...
	for ( ; ; )
	{
	    ...
	    else if ( letter == 'C') /* clan ownership */
		pRoomIndex->clanowner = fread_number(fp);

/*              
            else if ( letter == 'C' ) 
             {
               if (pRoomIndex->clanowner)
                 {
...
/* WIZZLE - VARIABLES USED FOR LOGGING BUG EXITS =
...
/* WIZZLE REMOVED SO NOT IN .LOG FILES 
...
void area_update( void )
{
	...
	{
	    ROOM_INDEX_DATA *pRoomIndex;
		ROOM_INDEX_DATA *pRoomIndexGauntlet;

	    reset_area( pArea );
	
	    pArea->age = number_range( 0, 3 );
	    pRoomIndex = get_room_index( ROOM_VNUM_SCHOOL );
		pRoomIndexGauntlet = get_room_index( ROOM_VNUM_GAUNTLET_RETURN );

	    if ( ( pRoomIndex != NULL && pArea == pRoomIndex->area ) ||
			(pRoomIndexGauntlet != NULL && pArea == pRoomIndexGauntlet->area))
...
/* */
		...
		if ( pObjIndex->count < limit || number_range(0,4) == 0 )
		{
		    ...
		    if (pObj->level > LastMob->level + 100
		    ||  (pObj->item_type == ITEM_WEAPON 
		    &&   pReset->command == 'E' 
		    &&   pObj->level < LastMob->level +50 && pObj->level < 0))
			fprintf(stderr,
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	/* computed on the spot */
        ...
        if (IS_SET(mob->act,ACT_ASSASSIN))
        {
        ...
        if (IS_SET(mob->act,ACT_ANTIPALADIN))
        {
            mob->perm_stat[STAT_STR] += 3;
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    obj->material_type	= pObjIndex->material_type;
...
void clone_object(OBJ_DATA *parent, OBJ_DATA *clone)
{
    ...
    clone->short_descr 	= str_dup(parent->short_descr);
    clone->description	= str_dup(parent->description);
    clone->material_type	= parent->material_type;
...
CLAN_DATA *get_clan_index( int vnum )
 {
  ...
  for ( pClanIndex = clan_first; pClanIndex; pClanIndex = pClanIndex->next )
...
void do_areas( CHAR_DATA *ch, char *argument )
{
 ...
 sprintf( buf, "\n\r{c[{C%-29s{c] {c[{C%-9s{c] {c[{C%-20s{c]{x\n\r", "Area Name", "LEVEL RNG","Builders"
);
 add_buf(buffer1,buf);
     ...
     if(!IS_IMP(ch))
       {
        ...
        if (ch->level <= 102)
          {
           if(((!IS_SET(pArea->area_flags, AREA_PROTO))
           && (!IS_SET(pArea->area_flags, AREA_GODS))
           && (!IS_SET(pArea->area_flags, AREA_IMP)))
           && (IS_SET(pArea->area_flags, AREA_PLAYER)))
             {
              sprintf( buf, "{c[{W%-29.29s{c] {c[{W%3d {c- {W%3d{c] {c[{W%-20.20s{c]{x\n\r",
               pArea->name,pArea->min_level, pArea->max_level,pArea->builders);
              add_buf(buffer1,buf);
             }
          }
        else
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "                {cAreas{w:  {W%-5d{x\n\r", top_area      ); send_to_char( buf, ch );
    sprintf( buf, "          {cExtra DESCs{w:  {W%-5d{x\n\r", top_ed        ); send_to_char( buf, ch );
    sprintf( buf, "                {cExits{w:  {W%-5d{x\n\r", top_exit      ); send_to_char( buf, ch );
    sprintf( buf, "                {cHelps{w:  {W%-5d{x\n\r", top_help      ); send_to_char( buf, ch );
    sprintf( buf, "              {cSocials{w:  {W%-5d\n\r", social_count  ); send_to_char( buf, ch );
    sprintf( buf, " {cTotal Different MOBs{w:  {W%-5d {D({C%5d {cNEW format{D)\n\r",
top_mob_index,newmobs); 
    send_to_char( buf, ch );
    sprintf( buf, "{cMOBs Currently in Use{w:  {W%-5d\n\r", mobile_count  ); send_to_char( buf, ch );
    sprintf( buf, " {cTotal Different OBJs{w:  {W%-5d {D({C%5d {cNEW format{D)\n\r",
top_obj_index,newobjs); 
    send_to_char( buf, ch );
    sprintf( buf, "               {cResets{w:  {W%-5d\n\r", top_reset     ); send_to_char( buf, ch );
    sprintf( buf, "                {cRooms{w:  {W%-5d\n\r", top_room      ); send_to_char( buf, ch );
    sprintf( buf, "                {cShops{w:  {W%-5d\n\r", top_shop      ); send_to_char( buf, ch );
    sprintf( buf, "              {cStrings{w:  {W%-5d {CStrings of {W%7d {CBytes {D({CMAX{w: {W%7d{D)\n\r",
	nAllocString, sAllocString, MAX_STRING );
    send_to_char( buf, ch );

    sprintf( buf, "                {cPERMs{w:  {W%d {CBlocks of {W%d {CBytes\n\r",
	nAllocPerm, sAllocPerm );
    send_to_char( buf, ch );

    sprintf( buf, "          {RREMORT ROOM{w:  {W%-5d{x\n\r",remvalue); 
    send_to_char(buf, ch );

    sprintf( buf, "      {RDragonPIT's Ran{w:  {W%-2d{x\n\r",dpran); 
    send_to_char(buf, ch );
...
*/
    ...
    /* open file 
    fclose(fpReserve);
    fp = fopen("/home/dragon/kotrd/files/dbdump/mem.dmp","w");

    num_pcs = 0;
    aff_count = 0;

    fprintf(fp,"MOB Prototypes: %5d (%8d bytes)\n",
    ...
    fprintf(fp,"OBJ Prototypes: %5d (%8d bytes)\n",
    ...
    fprintf(fp,"       Objects: %5d (%8d bytes), %2d free (%d bytes)\n",
    ...
    fprintf(fp,"         ROOMs: %5d (%8d bytes)\n",
	top_room, top_rooM * (sizeof(*room)));

    fprintf(fp,"         EXITs: %5d (%8d bytes)\n",
  ...
  */
    ...
    fp = fopen("/home/dragon/kotrd/files/dbdump/mob.dmp","w");

    fprintf(fp,"\n VNUM  LeVL  ACT  KIL   NAME                            S_DESC\n");
    fprintf(fp,
"------------------------------------------------------------------------------------------------\n");
    nMatch = 0;
	...
	{
	    nMatch++;
	    fprintf(fp,"%5d  %4d  %3d  %3d   %-30.30s  %-40.40s\n",
		pMobIndex->vnum,pMobIndex->level,pMobIndex->count,
		pMobIndex->killed,pMobIndex->player_name,strip_color(pMobIndex->short_descr));
	}
    fclose(fp);

    /* start printing out object data */
    fp = fopen("/home/dragon/kotrd/files/dbdump/obj.dmp","w");

    fprintf(fp,"\n VNUM  LeVL  ACT  RST   NAME                            S_DESC\n");
    fprintf(fp,
"------------------------------------------------------------------------------------------------\n");
    nMatch = 0;
    for (vnum = 0; nMatch < top_obj_index; vnum++)
	if ((pObjIndex = get_obj_index(vnum)) != NULL)
	{
		...
		pObjIndex->reset_num,pObjIndex->name,strip_color(pObjIndex->short_descr));
	}
...
void load_clans( void )
{
  ...
  for ( ; ; )
    ...
    pClanIndex        			= new_clan_index( );
    pClanIndex->vnum    		= vnum;
    pClanIndex->loners      = fread_number( fp );
    pClanIndex->name    		= fread_string( fp );
    pClanIndex->who_name    = fread_string( fp );
    pClanIndex->mort_leader   = fread_string( fp );
    pClanIndex->lieutenant   = fread_string( fp );
    pClanIndex->description = fread_string( fp );
    pClanIndex->hall  	  	= fread_number( fp );
    pClanIndex->pks_up	  	= fread_number( fp );
    pClanIndex->pks_dwn	  	= fread_number( fp );
    pClanIndex->pkd_up	  	= fread_number( fp );
    pClanIndex->pkd_dwn	  	= fread_number( fp );
    pClanIndex->members	  	= fread_number( fp );
    {
    	...
    	fBootDb = FALSE;
    	fpArea = NULL;
      members = fread_string( fp );
    	for ( ; ; )
    	  {
    	  members = one_argument( members, name );
    	  if ( name[0] == '\0' )
    	    break;
    	  pMember = fread_pkinfo( name );
    	  if ( !pMember )
    	    continue;
        sort_members( pClanIndex, pMember );
    	  }
    	}
    fpArea = fp;
...
void bug( const char *str, int param )
{
	...
	sprintf( buf, "[*****] FILE: %s LINE: %d %d", strArea, iLine, iChar );
	log_string( buf );
...
void load_maxes( void )
{
  FILE *fp;
  if ( !(fp=fopen(MAXES_FILE,"r")) )
    {
    atm_players = 0;
    date_atmp = str_dup( "" );
    return;
    }
  atm_players = fread_number( fp );
  date_atmp   = fread_string( fp );
  return;
}
