...
{
  int			i;
  struct hash_link	*scan,*temp,*gman;
...
void *hash_remove(struct hash_header *ht,int key)
{
  ...
  if(*scan)
	...
	if(ht->keylist[i]==key)
      ...
      if(i<ht->klistlen)
	{
	  bcopy((char *)ht->keylist+i+1,(char
*)ht->keylist+i,(ht->klistlen-i)
		*sizeof(*ht->keylist));
	  ht->klistlen--;
	}
...
void hash_iterate(struct hash_header *ht,void *cdata)
{
  int i;
