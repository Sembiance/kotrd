...
void load_objects( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        if ( letter != '#' )
        {
            bug( "Load_objects: # not found. (%c)", letter );
            exit( 1 );
        }
...
/*        pObjIndex->material_type	= material_lookup(fread_word(fp)); */

CHECK_POS(pObjIndex->material_type, material_lookup(fread_word( fp )), "material_type" );

	CHECK_POS(pObjIndex->item_type, item_lookup(fread_word( fp )), "item_type" );
        pObjIndex->extra_flags          = fread_flag( fp );
	        //if(IS_SET(pObjIndex->extra_flags, ITEM_LEGEND))
	         //  bug("LEGEND FOUND: Vnum %d", vnum);
