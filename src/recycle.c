...
PC_DATA *new_pcdata(void)
{
    ...
    *pcdata = pcdata_zero;
    ...
    // Stones of Wisdom Stuff
    pcdata->stones = (STONES_DATA *)malloc(sizeof(STONES_DATA));
    stones_clear(pcdata->stones);
...
CLAN_DATA *		new_clan(void)
{
	...
	clan->next=NULL;
	clan->prev=NULL;
	clan->name=str_dup("no name");
	clan->full_name=str_dup("no full name");
	clan->who_text=str_dup("no who text");
...
void	free_clan(CLAN_DATA *clan)
{
	if(clan->members)
	{
		log_string("ERROR: Free your members, before freeing the clan.");
		exit(1);
	}
	
	free_string(clan->name);
	free_string(clan->who_text);
	free_string(clan->full_name);
