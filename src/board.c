...
{ "General",  	"General discussion",            0,     2,              "all", DEF_INCLUDE,60, NULL, FALSE },
{ "Ideas",	  	"Suggestion for improvement",	 0,     2,              "all", DEF_NORMAL, 365, NULL, FALSE }, 
{ "Announce", 	"Announcements from Immortals",  0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 365, NULL, FALSE },
{ "Personal", 	"Personal messages",			 0,     1,              "all", DEF_EXCLUDE,180, NULL, FALSE }
...
sh_int get_trust(CHAR_DATA *ch) { return ch->level; }
...
void finish_note (BOARD_DATA *board, NOTE_DATA *note)
{
	...
	// Now go through all online players and send notices
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
            ...
            &&  is_note_to(d->character, note))
            {
                ...
                send_to_char("{GA new note has been posted to you!\n\r", d->character);
            }
    }	
}
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument || !argument[0]) /* show boards */
	{
		int unread;
