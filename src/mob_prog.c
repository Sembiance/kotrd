...
#define CHK_EXPLORED_PERCENT    (51)
#define CHK_KILLED_PERCENT      (52)
#define CHK_FOUND_PERCENT       (53)
...
const char * fn_keyword[] =
{
    ...
    "mobs",		    /* 7 if mobs > 2		- does room contain > 2 mobiles */
    ...
    "isgood",		/* 13 if isgood $n 	- is $n good */
    "isevil",		/* 14 if isevil $n 	- is $n evil */
    "isneutral",	/* 15 if isneutral $n 	- is $n neutral */
    "isimmort",		/* 16 if isimmort $n	- is $n immortal */
    "ischarm",		/* 17 if ischarm $n	- is $n charmed */
    "isfollow",		/* 18 if isfollow $n	- is $n following someone */
    "isactive",		/* 19 if isactive $n	- is $n's position > SLEEPING */
    "isdelay",		/* 20 if isdelay $i	- does $i have mobprog pending */
    "isvisible",	/* 21 if isvisible $n	- can mob see $n */
    "hastarget",	/* 22 if hastarget $i	- does $i have a valid target */
    "istarget",		/* 23 if istarget $n	- is $n mob's target */
    "exists",		/* 24 if exists $n		- does $n exist somewhere */

    "affected",		/* 25 if affected $n blind - is $n affected by blind */
    "act",		    /* 26 if act $i sentinel	- is $i flagged sentinel */
    "off",          /* 27 if off $i berserk	- is $i flagged berserk */
    "imm",          /* 28 if imm $i fire	- is $i immune to fire */
    "carries",		/* 29 if carries $n sword	- does $n have a 'sword' */
			        /* if carries $n 1233	- does $n have obj vnum 1233 */
    "wears",		/* 30 if wears $n lantern	- is $n wearing a 'lantern' */
			        /* if wears $n 1233	- is $n wearing obj vnum 1233 */
    "has",    		/* 31 if has $n weapon	- does $n have obj of type weapon */
    "uses",		    /* 32 if uses $n armor	- is $n wearing obj of type armor */
    "name",		    /* 33 if name $n puff	- is $n's name 'puff' */
    "pos",		    /* 34 if pos $n standing	- is $n standing */
    "race",		    /* 35 if race $n dragon	- is $n of 'dragon' race */
    "class",		/* 36 if class $n mage	- is $n's class 'mage' */
    "objtype",		/* 37 if objtype $p scroll	- is $p a scroll */

    "vnum",		    /* 38 if vnum $i == 1233  	- virtual number check */
    "hpcnt",		/* 39 if hpcnt $i > 30	- hit point percent check */
    "room",		    /* 40 if room $i == 1233	- room virtual number */
    "sex",		    /* 41 if sex $i == 0	- sex check */
    "level",		/* 42 if level $n < 5	- level check */
    "align",		/* 43 if align $n < -1000	- alignment check */
    "money",		/* 44 if money $n */
    "objval0",		/* 45 if objval0 > 1000 	- object value[] checks 0..4 */
    "objval1",      /* 46 */
    "objval2",      /* 47 */
    "objval3",      /* 48 */
    "objval4",      /* 49 */
    "grpsize",		/* 50 if grpsize $n > 6	- group size check */

    "exploredpercent", /* 51 if exploredpercent $n >= 90 - Check what percentage of rooms they have explored */
    "killedpercent",   /* 52 if killedpercent $n >= 90 - Check what percentage of mobs they have killed */
    "foundpercent",    /* 53 if foundpercent $n >= 90 - Check what percentage of objects they have found */
...
int get_explored_percent(CHAR_DATA * ch)
{
    ...
    for(pArea=area_first;pArea;pArea=pArea->next)
    {
        ...
        if(totalRoomsInArea==0)
            continue;
        
        totalRoomsExplored+=exploredInThisArea;
    }

    return ((totalRoomsExplored*100)/gTotalRooms);
}

int get_killed_percent(CHAR_DATA * ch)
{
    AREA_DATA *     pArea;
    char **         ar=0;          
    ...
    for(pArea=area_first;pArea;pArea=pArea->next)
    {
        ...
        if(totalMobsInArea==0)
            continue;
        
        totalMobsKilled+=killedInThisArea;
    }
     
    return ((totalMobsKilled*100)/gTotalMobs);
}

int get_found_percent(CHAR_DATA * ch)
{
    AREA_DATA *     pArea;
    char **         ar=0;          
    ...
    for(pArea=area_first;pArea;pArea=pArea->next)
    {
        ...
        if(totalObjectsInArea==0)
            continue;
        
        totalObjectsFound+=foundInThisArea;
     }
     
    return ((totalObjectsFound*100)/gTotalObjects);
}
...
bool has_item( CHAR_DATA *ch, sh_int vnum, sh_int item_type, bool fWear )
{
	...
	&&   ( !fWear || obj->wear_loc != WEAR_INVENTORY ) )
	    return TRUE;
    return FALSE;
}
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_EXPLORED_PERCENT:
	    if( lval_char != NULL ) lval = get_explored_percent(lval_char); break;
	case CHK_KILLED_PERCENT:
	    if( lval_char != NULL ) lval = get_killed_percent(lval_char); break;
	case CHK_FOUND_PERCENT:
	    if( lval_char != NULL ) lval = get_found_percent(lval_char); break;	    
	default:
            return FALSE;
    }
    return( num_eval( lval, oper, rval ) );
...
#define MAX_NESTED_LEVEL 30 /* Maximum nested if-else-endif's (stack size) */
