 ...
 */
DECLARE_SPEC_FUN(	spec_draglord		);
...
bool spec_patrolman(CHAR_DATA *ch)
{
	...
	case 2: message = 
		...
		break;
	case 3: message = "$n yells 'Stop that! Stop that!' and attacks.";
		break;
    ...
    if (ch->position != POS_FIGHTING) {
       for ( victim = ch->in_room->people; victim != NULL; victim = v_next)
       {
	     ...
	     do_function(ch, &do_backstab, victim->name);
            ...
            if (ch->position != POS_FIGHTING)
	     {
                 do_function(ch, &do_assassinate, victim->name);
	     }
...
bool spec_executioner( CHAR_DATA *ch )
{
    ...
    sprintf( buf, "{W%s {cis a {R%s{c!  {WPROTECT THE INNOCENT{c!  {WMORE {RBLOOOOD{c!!!{x",
	victim->name, crime );
...
bool spec_draglord (CHAR_DATA *ch)
{
    if (ch->fighting != NULL) 
    return spec_cast_mage( ch );
    return FALSE;
